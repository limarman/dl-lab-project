...
class TestSimpleState(unittest.TestCase):
    ...
    def test_get_tensor(self):
        tensor_shape = self.advanced_state.tensor.shape
        self.assertEqual(list(tensor_shape), [2*21*21 + 8*1])

    def test_apply_action_to_board(self):
        next_state = self.advanced_state.apply_action_to_board([self.spawn_ship_action])

        # test that there is one ship more
        ship_count_previous = self.advanced_state.ship_count_me
        ...
        kore_previous = self.advanced_state.kore_me
